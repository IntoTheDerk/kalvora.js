/**
 * Kalvora Wallet Creation Constants
 * 
 ...
 * 
 * BIP44 PATH:     m/44/5258/0/0/0 (mixed hardened/normal)
 * OUR PATH:       m/44'/5258'/0'/0'/0' (all hardened)
 * 
...
// Kalvora's proposed SLIP-0044 coin type. The value encodes the distinctive
// KALV name prefix on a telephone keypad: K=5, A=2, L=5, V=8.
export const KALVORA_TYPE = 5258;
export const KALVORA_TYPE_HEX = '0x8000148a';
export const KALVORA_SYMBOL = 'KAL';
export const KALVORA_NAME = 'Kalvora';
...
export const SLIP0010_DERIVATION_PATH = `m/44'/${KALVORA_TYPE}'/0'/0'/0'`;
...
// Kalvora extended key version bytes (custom to prevent cross-chain confusion)
export const EXTENDED_KEY_VERSIONS = {
  PRIVATE: 0x04b2430c, // Kalvora private key version
  PUBLIC: 0x04b2430d   // Kalvora public key version
} as const;
...
export function validateSLIP0010Path(path: string): boolean {
  ...
  try {
    ...
    // Coin type must be Kalvora (5258)
    if (parseInt(parts[2]?.slice(0, -1) || '0') !== KALVORA_TYPE) return false;
